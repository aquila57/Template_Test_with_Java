// SinTempl.java - Template Test with bad data  Version 0.1.0
...
//**********************************************************
// This program is intended to produce a failing
// chi square test at the end.
// The random number generator is distorted by
// the sine function.
// The template pattern is uniform.
// The samples are generated as follows:
// A random number x is generated from zero to Pi/2.
// y = sin(x)
// The number y is a random number from zero to one,
// but y is not have a uniform distribution.
   ...
   public int takeSamples()
      ...
      int status;
		...
		for (i=0;i<samples;i++)
		   {
		   popActual();
			// x is a real number from zero to Pi/2 radians
			x = et.genunif() * halfpi;
			// y is a real number from zero to one
			// y does not have a uniform distribution
			y = Math.sin(x);
			if (y >= 0.5)
			   {
				num = 1;
   ...
   public void printHeading()
	   {
		System.out.println();
		System.out.print("            ");
		System.out.println("      Template Test with Sine");
		System.out.print("            ");
