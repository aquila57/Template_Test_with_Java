// WrapTmpl.java - FIFO Queue Template Test  Version 0.1.0
...
//**********************************************************
// This program is intended to generate a wrap-around
// error.
// A wrap-around error occurs, when all 1024 nodes
// in the template queue are matched.
// The sample number for the wrap-around error
// message is 500000 plus 1024,
// because the template is included in the period
// length of the random number generator.
// After taking 500 thousand samples, this program
// re-initializes the random number generator with
// the same seeds as before.
// This is an artificial means for forcing a wrap-around
// error.
...
public class WrapTmpl
	...
	WrapTmpl()
	   {
		...
		// to a fixed state.
		// Normally it is initialized to a random state.
		// Etaus will be re-initialized to the same
		// fixed state in the method, takeSamples.
		//********************************************************
		parm[0] = 123456789;
		parm[1] = parm[0] + 17;
		parm[2] = parm[0] + 31;
   ...
   // generate one million rolling sample queues.
	...
	// In this version of the template test,
	// the random number generator is re-initialized
	// to a fixed state after 500 thousand samples
	// have been taken.
	// A wrap-around error will occur after another
	// 1024 samples have been taken.
   ...
   public int takeSamples()
	   {
		...
		for (i=0;i<samples;i++)
		   {
			//***********************************************
			// After 500 thousand samples,
			// repeat the random number generator
			// to create a wrap-around error.
			//***********************************************
			if (i == 500000)
			   {
				int parm[] = new int[8];
